...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 ...
 //----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        {
            // 🛠️ Car ID மூலம் car-ஐ database-ல் இருந்து தேடுகிறது
            var car = dbContext.Cars.FirstOrDefault(c => c.CarId == id);
            if (car == null)
            {
                // 🛑 Car கிடைக்கவில்லை என்றால் 404 error
                return NotFound();
            }

            // 🗑️ Car-ஐ database-ல் இருந்து நீக்குகிறது
            dbContext.Cars.Remove(car);
            await dbContext.SaveChangesAsync();

            // 🔄 Car list page-க்கு redirect செய்கிறது
